...
def set_map_tick_labels(fontsize=15):
    ...
    plt.yticks(
        [int(limite) for i_limite, limite in enumerate(np.linspace(y_min, y_max, 5)) if i_limite > 0],
        [
    ...
    plt.xticks(
        [int(limite) for i_limite, limite in enumerate(np.linspace(x_min, x_max, 5)) if i_limite > 0],
        [
...
def set_scale_bar(ax,length,width,loc='lower right'):
    scalebar = AnchoredSizeBar(ax.transData,length, f'{length} m', loc,pad=0.1,color='black',frameon=False,size_vertical=width)
...
def plot_location_plot(ax, linea_costa, margen_x, margen_y, box_length = 500):
    ...
    axins.set_yticks([])
    axins.plot(
        [margen_subplot_x[1], margen_subplot_x[0], margen_subplot_x[0], margen_subplot_x[1], margen_subplot_x[1]],
        [margen_subplot_y[0], margen_subplot_y[0], margen_subplot_y[1], margen_subplot_y[1], margen_subplot_y[0]],
        "red",
        linewidth=2,
    )
